...
namespace TypographyFileImplement.Implements
{
    public class WarehouseStorage : IWarehouseStorage
    {
        ...
        public bool IsTaked(Dictionary<int, (string, int)> components, int printedCount)
        {
            foreach (var warehouseComponent in components)
            {
                ...
                if (count < warehouseComponent.Value.Item2 * printedCount)
                {
                    return false;
                }
            }
            ...
            foreach (var warehouseComponent in components)
            {
                int count = warehouseComponent.Value.Item2 * printedCount;
