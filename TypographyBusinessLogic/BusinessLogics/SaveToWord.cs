           ...
           WordprocessingDocument.Create(info.FileName, WordprocessingDocumentType.Document))
            {
                ...
                foreach (ViewModels.PrintedViewModel printed in info.Printeds)
                {
                    docBody.AppendChild(CreateParagraph(new WordParagraph
                    {
                        Texts = new List<(string, WordTextProperties)> { (printed.PrintedName + ": ", new WordTextProperties { Bold = true, Size = "24", }), (printed.Price.ToString(), new WordTextProperties { Size = "24" }) },
                        TextProperties = new WordTextProperties
                        {
                            Size = "24",
